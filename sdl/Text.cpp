...
{
	surface = std::shared_ptr<SDL_Surface>(SDL_CreateRGBSurfaceWithFormat(0,
				width, height, 32, SDL_PIXELFORMAT_ARGB8888), SDL_FreeSurface);
	if (surface.get() == NULL)
		throw std::runtime_error("SDL_CreateRGBSurface failed");
	cairo_surface = std::shared_ptr<cairo_surface_t>(cairo_image_surface_create_for_data(static_cast<unsigned char *>(surface->pixels),
				CAIRO_FORMAT_ARGB32, surface->w, surface->h, surface->pitch),
			cairo_surface_destroy);
	if (cairo_surface.get() == NULL)
		throw std::runtime_error("cairo_image_surface_create_for_data failed");
	cairo_context = std::shared_ptr<cairo_t>(cairo_create(cairo_surface.get()),
			cairo_destroy);
	if (cairo_context.get() == NULL)
...
void Text::update_text()
{
	...
	cairo_select_font_face(cairo_context.get(), font.c_str(),
...
void Text::draw(SDL_Renderer *renderer, int px, int py)
{
	...
	rect = {px, py, surface->w, surface->h};
